...
import chapter18.config.KafkaConfig;
import chapter18.config.MysqlConfig;
import chapter18.config.RedisConfig;
import chapter18.metrics.ApiMetrics;
import chapter18.metrics.DbMetrics;
import chapter18.scheduled.ScheduledUpdater;
import chapter18.util.ConfigSource;
import chapter18.util.ZookeeperConfigSource;
import chapter18.view.SimpleHttpServer;
 ...
 * 为了在内存中存储这些配置信息，供项目中的其他模块来使用，我们分别设计实现了三个 Configuration 类：
 ...
 *  Kafka 热更新，不暴露配置
 *
 * 不设计一个大而全的 Config 接口 ，而是设计 Updater 和 Viewer 两个小接口。 是为了满足 接口隔离原则。
 *
 *
 * 接口隔离的好处：
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 新增 ScheduledUpdater 定时任务类 和 Updater 接口。 需要更新的配置类 去实现该热更新接口。
