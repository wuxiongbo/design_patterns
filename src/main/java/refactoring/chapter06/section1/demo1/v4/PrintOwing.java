 ...
 * <p>
 * 被赋值的临时变量也分两种情况:
 * 1)较简单的情况是：这个变量只在被提炼代码段中使⽤。
 * 果真如此，你可以将这个临时变量的声明移到被提炼代码段中，然后⼀起提炼出去。
 * 2)另⼀种情况是：被提炼代码段之外的代码也使⽤这个变量。
 * 这⼜分为两种情况：
 * 2.1)如果 这个变量在被提炼代码段之后未再被使⽤，你只需直接在⽬标函数中修改它就可以了；
 * 2.2)如果 被提炼代码段之后的代码还使⽤了这个变量，你就需要让⽬标函数返回该变量改变后的值。
 * <p>
 * 我以下列代码说明这⼏种不同情况：
 *
 * @author bear
...
public class PrintOwing {
    ...
    /**
     * double变量 outstanding 在被提炼代码段外被⽤到，所以, 必须让提炼出来的新函数返回它。
     */
