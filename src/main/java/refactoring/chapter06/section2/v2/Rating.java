 ...
 * 你⼿上有⼀群组织不甚合理的函数。
 * 你可以将它们都内联到⼀个⼤型函数中，再从中提炼出组织合理的⼩型函数。
 * Kent Beck发现，实施 Replace Method with Method Object （135）之前 先这么做，往往可以获得不错的效果。
 * 你可以把所要的函数（有着你要的⾏为）的所有调⽤对象的函数内容都内联到函数对象中。
 * ⽐起 既要 移动⼀个函数 又要 移动它所调⽤的其他所有函数，将整个⼤型函数作为整体来移动会⽐较简单。
 * 如果别⼈使⽤了太多间接层，使得系统中的所有函数都似乎只是对另⼀个函数的简单委托，造成我在这些委托动作之间晕头转向，
