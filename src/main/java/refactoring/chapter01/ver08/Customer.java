 ...
 * 这听起来很像子类的工作。我们可以建立Movie 的三个子类，每个都有自己的计费法
 *
 *
 *
 * 把因条件而异的代码替换掉。
 * 条件中，调用 谁 的属性判断，该代码 就迁移给 谁
 * <p>
 * <p>
 * 新需求：用户准备修改影片分类规则
 * <p>
 * (系统运行前)一部影片(Movie类)可以在生命周期内修改自己的分类(修改代码)，(系统运行后)一个对象(Movie实例)却不能在生命周期内修改自己所属的类
 * <p>
 * 比如：
 ...
 * <p>
 * Movie 对象创建之后，任何时候调用 getCharge() 都是调用 Movie 的 getCharge()方法。
