...
/**
 * <p>
 * 4) 现在，我可以运用 与3) 同样的手法，处理 {@link Rental#getFrequentRenterPoints()},
 *    将 {@link Movie#getFrequentRenterPoints(int) } 逻辑 搬移至 {@link Price#getFrequentRenterPoints(int)}。
 *    但是这一次，我不把超类函数({@link Price#getFrequentRenterPoints(int)})声明为 abstract。
 *    我只是为 新片类型({@link ChildrensPrice#getPriceCode()}) 增加一个 覆写函数，
 *    并在超类({@link Price})内，留下一个已定义的函数{@link Price#getFrequentRenterPoints(int)}，使它成为一种默认行为。
 * <p>
 * =============================
 * <p>
 * 引入 State模式 花了我不少力气，值得吗?
 * 这么做的收获是：如果我要修改任何与 价格 Price 有关的行为，或是 添加 新的定价标准，或是加入 其他取决于价格的行为，程序的修改会容易得多。
 * 这个程序的其余部分，并不知道我运用了 State模式。
 * 对于我目前拥有的这么几个小量行为来说，任何 功能 或 特性 上的修改，也许都不划算，
 ...
 * 不过，一旦你习惯了这种 重构后的风格，就很难再满足于 结构化风格 了。
 * <p>
 * 这个例子，给我们最大的启发是，重构的节奏：测试、小修改、测试、小修改、测试、小修改......
