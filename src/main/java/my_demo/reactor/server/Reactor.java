 ...
 *
 * 声明式编程   WHAT
 *   告诉计算机 应该做什么、想要什么结果，但不关注 具体要怎么做。
 ...
 * 响应式编程
 *   观察者模式、Reactor设计模式。
 *   不主动调用某个请求的API，而是通过， 事先 注册对应 接口，在未来某个时刻，再通过 事件 回过头来 触发执行该接口
 *   如：excel 计算函数。
 *      事先 将计算函数 写好。 输入数据流，回来调用 预先写好的函数， 得到动态变动的结果
 *
 * 面向对象编程
 *   从问题域出发，将封装、继承、多态的语言特性映射到现实世界。屏蔽计算机底层。
 *   对象 作为程序的基本组成单元
 *
 *
 * <pre>
